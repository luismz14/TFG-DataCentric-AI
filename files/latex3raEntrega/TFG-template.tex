...
En la configuración actual se utilizan tres filtros:

\begin{itemize}
    \item \textbf{Oscuridad:} detecta imágenes con iluminación insuficiente. La imagen se transforma al espacio HSV y se calcula la media del canal V. Si este valor es inferior a $T_{oscuro}=60$, el frame se descarta.
    \item \textbf{Uniformidad:} elimina imágenes con muy poca estructura visual, típicamente producidas por contacto de la cámara con la mucosa. Para ello se calcula la entropía de Shannon en escala de grises. Si la entropía es inferior a $T_{uniform}=6.3$, el frame se descarta.
    \item \textbf{Desenfoque:} detecta imágenes borrosas mediante la varianza del Laplaciano. Si la varianza es inferior a $T_{lap}=25$, la imagen se considera insuficientemente nítida y se elimina.
\end{itemize}

\subsubsection{Selección de umbrales}
Los umbrales se seleccionan a partir de la distribución empírica de cada métrica y de la inspección visual de ejemplos cercanos a distintos puntos de corte. Para cada filtro se comparan varios valores candidatos alrededor del umbral final: 55, 60 y 65 para oscuridad; 6.0, 6.3 y 6.5 para uniformidad; y 25, 30 y 35 para desenfoque. Esta comparación permite equilibrar dos riesgos opuestos: conservar imágenes sin valor diagnóstico o eliminar frames que todavía contienen información útil.

La Figura~\ref{fig:phase4_threshold_histograms} muestra la distribución de las tres métricas y los umbrales candidatos evaluados. Los umbrales seleccionados se sitúan en zonas de transición de la distribución, evitando cortes excesivamente agresivos sobre la masa principal de imágenes.

\begin{center}
\begin{minipage}{\linewidth}
\centering
\includegraphics[width=\linewidth]{img/phase4_threshold_histograms.png}
\captionof{figure}{Distribución de las métricas utilizadas en el filtrado de calidad. La línea de mayor grosor marca el umbral seleccionado para cada filtro.}
\label{fig:phase4_threshold_histograms}
\end{minipage}
\end{center}

Además de la distribución numérica, se revisan ejemplos cercanos al umbral seleccionado para comprobar que la decisión tenga sentido visual. La Figura~\ref{fig:phase4_threshold_examples} muestra imágenes próximas a los tres puntos de corte utilizados, lo que permite validar que los umbrales separan casos de baja calidad sin convertir el filtrado en una eliminación excesivamente restrictiva.

\begin{center}
\begin{minipage}{\linewidth}
\centering
\includegraphics[width=\linewidth]{img/phase4_threshold_examples.png}
\captionof{figure}{Ejemplos de frames cercanos a los umbrales seleccionados para oscuridad, uniformidad y desenfoque.}
\label{fig:phase4_threshold_examples}
\end{minipage}
\end{center}
